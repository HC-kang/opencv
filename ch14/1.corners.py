...
def corner_fast():
    ...
    fast = cv2.FastFeatureDetector_create(60)      # 밝기 차이 임계값은 60
    keypoints = fast.detect(src)                   # 비최대억제 수행. 검출된 모든 코너점 좌표 저장

    dst = cv2.cvtColor(src, cv2.COLOR_GRAY2BGR)    # src 영상을 3채널 컬러로 변환하여 dst에 저장

    for kp in keypoints:
        pt = (int(kp.pt[0]), int(kp.pt[1]))
        cv2.circle(dst, pt, 5, (0, 0, 255), 2)     # 검출된 모든 코너 점에 반지름 5인 빨간 원
